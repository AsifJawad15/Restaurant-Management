<?php
/**
 * Admin Dashboard - OOP Implementation
...
// Get dashboard statistics using our OOP models
try {
    ...
    // Total revenue today
    $todaysRevenue = $orderModel->getTodaysRevenue();
    
    // Total menu items
    $totalMenuItems = $menuItemModel->getActiveCount();
    
    // Total categories
    $totalCategories = $categoryModel->getActiveCount();
    
    // Featured items count
    $featuredItems = $menuItemModel->getFeaturedCount();
    
    // Pending orders
    $pendingOrders = $orderModel->getPendingOrderCount();
    
    // Recent orders with customer details
    $recentOrders = $orderModel->getRecentOrdersWithCustomers(10);
    
    // Popular menu items (last 7 days)
    $popularItems = $menuItemModel->getPopularItems(5, 7);
    
} catch (Exception $e) {
    $error = "Dashboard error: " . $e->getMessage();
}

// Helper function for price formatting (keep existing function)
function formatPrice($amount) {
    return '$' . number_format((float)$amount, 2);
}

// Helper function for date formatting (keep existing function)
                    ...
                    <div class="col-12">
                        <div class="alert alert-success border-0">
                            <h5 class="alert-heading">
                                ...
                                Welcome back, <?php echo $_SESSION['admin_name'] ?? 'Admin'; ?>!
                            </h5>
